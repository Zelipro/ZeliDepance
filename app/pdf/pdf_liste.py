# --- PDF LISTE : Génération PDF/HTML pour une liste de dépenses ---

import os
import importlib
from html import escape

from app.db.depenses import get_depenses_by_liste
from app.db.listes import get_liste_name
from app.pdf import has_reportlab, get_pdf_dir


def _build_html_liste(liste_nom: str, depenses: list[tuple], total: float) -> str:
    rows = []
    ...
    rows_html = "\n".join(rows) if rows else "<tr><td colspan='4'>Aucune depense</td></tr>"
    return f"""<!DOCTYPE html>
<html lang='fr'>
<head>
  <meta charset='UTF-8'>
  <title>Depenses - {escape(liste_nom)}</title>
  <style>
...
<body>
  <h1>Depenses - {escape(liste_nom)}</h1>
  <table>
    <thead><tr><th>Description</th><th>Montant</th><th>Categorie</th><th>Date</th></tr></thead>
    <tbody>
      {rows_html}
    </tbody>
  </table>
...
def generate_pdf_liste(liste_id: int, file_path: str = None) -> str:
    """
    Génère un rapport PDF (ou HTML fallback) pour une liste de dépenses.
    ...
    total = sum(float(d[2]) for d in depenses)

    # ── Chemin de sortie automatique ──────────────────────────────────────
    if file_path is None:
        ext = ".pdf" if has_reportlab() else ".html"
        # Nettoyer le nom de liste pour le nom de fichier
        safe_nom = "".join(c for c in liste_nom if c.isalnum() or c in (' ', '_', '-')).strip()
        safe_nom = safe_nom.replace(' ', '_') or "liste"
        file_path = os.path.join(get_pdf_dir(), f"depenses_{safe_nom}{ext}")
    ...
    if has_reportlab():
        ...
        elements.append(Paragraph(f"Depenses - {liste_nom}", styles["Title"]))
    ...
    with open(html_path, "w", encoding="utf-8") as f:
        f.write(_build_html_liste(liste_nom, depenses, total))
    return html_path
